...
#             'response': message,
#         }
#     # Insert this document into the 'user_responses' collection and get the result
#     result = mongo.db.user_responses.insert_one(user_responses)
#     print(f"Inserted response into database, received ID: {result.inserted_id}")
...
def process_message(phone_number, message):
    ...
    if user_responses:
        ...
        mongo.db.user_responses.update_one(
            {'phone_number': phone_number},
            {'$set': {f'response_{question_number}': message}}
        )
        ...
        user_responses = {
            'phone_number': phone_number,
            f'response_{question_number}': message,
        }
        result = mongo.db.user_responses.insert_one(user_responses)
